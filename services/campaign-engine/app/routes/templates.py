...
@router.get("")
async def list_templates_route(project_id: str, db: DbDep) -> list[dict]:
    return await list_templates(db, project_id)


@router.get("/{template_id}")
async def get_template_route(project_id: str, template_id: str, db: DbDep) -> dict:
...
async def delete_template_route(
    ...
    if not doc:
        raise HTTPException(status_code=404, detail="Template not found")
    await delete_template(db, project_id, template_id)
