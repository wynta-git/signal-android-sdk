...
import { openContextMenu, openDrawer } from '@/store/slices/uiSlice';

export default function PromoCodeRow({ code, configureId, highlight }) {
  ...
  useEffect(() => {
    const onHi = (e) => {
      if (e.detail && e.detail.codeId === code.id) {
        setOpen(true);
        setFlash(true);
        // Scroll the card to a comfortable position
        if (cardRef.current) {
          const rect = cardRef.current.getBoundingClientRect();
          const target = window.scrollY + rect.top - 120;
          window.scrollTo({ top: target, behavior: 'smooth' });
        }
        // Clear the flash after the highlight animation finishes
  ...
  const handleIssueBonus = (e) => {
    e.stopPropagation();
    dispatch(openDrawer({ type: 'ISSUE_CODE_BONUS', code, configureId }));
  };
