...
// ── Deterministic player generation (mirrors HTML source verbatim) ────────────
...
export function _initials(name) { return name.split(' ').map(s => s[0]).join('').slice(0, 2).toUpperCase(); }
export function _tierClass(t)   { return 'tier-' + t.toLowerCase().replace(/\s+/g, '-'); }
export function _kycClass(k)    { return 'kyc-' + k.toLowerCase(); }

// ─────────────────────────────────────────────────────────────────────────────

export default function SegmentPlayersList({ segment, onPickPlayer }) {
  ...
  const searchPool = useMemo(() => {
    ...
    const out = [];
    for (let i = 0; i < cap; i++) out.push(makePlayer(segment.id, i));
    return out;
  }, [segment.id, query ? '_' : '']); // eslint-disable-line

  useEffect(() => { setPage(1); }, [segment.id, query]);
  ...
  if (q && searchPool) {
    ...
    slice = [];
    for (let i = startIdx; i < endIdx; i++) slice.push(makePlayer(segment.id, i));
    scopeNote = '';
  }

  const safePage = Math.min(page, pageTotal);
  const startIdx = (safePage - 1) * PLAYERS_PAGE_SIZE;
    ...
    <div className="seg-players">
      ...
      {pageTotal > 1 && (
        <div className="seg-players-pager">
          ...
          <Pager page={safePage} totalPages={pageTotal} onChange={setPage}/>
        </div>
      )}
    </div>
  );
