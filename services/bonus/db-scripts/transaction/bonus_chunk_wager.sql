...
-- a bonus chunk. One row per wager event per chunk; the sum of wager_amount
-- for a given chunk_id equals bonus_chunk.wager_amount.
--
-- COLUMN GROUPS
-- ─────────────
-- Identity  : id, chunk_id, wager_ref
...
-- • release_amount is non-zero only on the wager row that tips the chunk
--   over the threshold and triggers the release.
...
-- Sample data
-- -----------------------------------------------------------------------------
INSERT INTO `bonus_chunk_wager`
    (`id`, `chunk_id`, `wager_ref`, `wager_amount`, `release_amount`)
VALUES
    -- chunk_id=1 (CH001, bonus_log_id=1) — two wagers totalling 50.00; second triggers release
