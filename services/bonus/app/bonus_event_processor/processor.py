...
async def _dispatch(
    ...
    if trigger.release_type == "BONUS_RELEASE":
        await handle_bonus_release(redis, conn, event, trigger)
    elif trigger.release_type == "CHUNK_RELEASE":
        await handle_chunk_release(conn, event, trigger)
    else:
        log.warning(
...
async def process_bonus_event(redis: Redis, event: dict[str, Any]) -> None:
    ...
    2. Load all active release triggers for the site (Redis-first, DB fallback).
    3. Filter to triggers whose trigger_type matches the event name.
    4. Open one DB connection and dispatch each matched trigger to its handler.
    """
    ...
    async with get_connection(POOL_BONUS) as conn:
        for trigger in matching:
            try:
                await _dispatch(redis, conn, event, trigger)
