...
@router.post("/token", response_model=TokenResponse)
async def get_system_token(body: TokenRequest) -> TokenResponse:
    result = await validate_client(body.username, body.password, get_redis())
    now = int(datetime.now(tz=timezone.utc).timestamp())
    payload = {
        "sub": result.client_id,
        "type": "system",
    ...
    log.info("system_token_issued", service=body.username)
    return TokenResponse(access_token=token, expires_in=settings.jwt_token_ttl)
