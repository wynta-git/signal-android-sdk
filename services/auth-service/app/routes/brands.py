...
from shared.services.client import get_all_projects
...
async def list_brands(ctx: PortalAuthDep) -> list[BrandResponse]:
    redis = get_redis()
    projects = await get_all_projects(redis)
    project = next((p for p in projects if p.project_key == ctx.project_id), None)
    if project is None:
