...
function channelLabel(ch: string): string {
  const map: Record<string, string> = {
    ...
    whatsapp: 'WhatsApp', telegram: 'Telegram', in_app: 'In-App',
  };
...
function exportCsv(rows: ChannelRow[]) {
  const header = 'Channel,Messages,Delivery Rate,Bounce Rate,Open Rate,CTR,Opt-outs';
  const lines  = rows.map(r =>
    `${channelLabel(r.channel)},${r.messages},${fmtPct(r.delivery_rate)},${fmtPct(r.bounce_rate)},${r.open_rate != null ? fmtPct(r.open_rate) : '—'},${r.ctr != null ? fmtPct(r.ctr) : '—'},${r.opt_outs != null ? r.opt_outs : '—'}`
  );
  const blob = new Blob([[header, ...lines].join('\n')], { type: 'text/csv' });
  const url  = URL.createObjectURL(blob);
  const a    = document.createElement('a');
  a.href = url; a.download = 'channel-delivery.csv'; a.click();
  ...
  const summaryCards: CardProps[] = [
    {
      ...
      value:  fmt(s?.total_messages?.value),
      change: s?.total_messages?.change_pct,
    },
    {
      label: 'Delivery Rate',
      value: s?.delivery_rate?.value != null ? fmtPct(s.delivery_rate.value) : '—',
    },
    {
      label: 'Bounce Rate',
      value: s?.bounce_rate?.value != null ? fmtPct(s.bounce_rate.value) : '—',
    },
    {
      label: 'Opt-outs (7d)',
      value: s?.opt_outs_7d?.value != null ? fmt(s.opt_outs_7d.value) : '—',
      sub:   s?.opt_outs_7d?.tracked === false ? 'Pending data' : undefined,
    },
    {
      label:    'Active Channels',
      value:    s?.active_channels?.value != null ? String(s.active_channels.value) : '—',
      sub:      s?.active_channels?.paused ? `${s.active_channels.paused} paused` : undefined,
      subColor: s?.active_channels?.paused ? 'var(--crm-negative)' : undefined,
    },
  ];
      ...
      <div style={{ padding: '18px 28px 14px', borderBottom: '1px solid var(--crm-border)', background: 'var(--crm-white)' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                <circle cx="4"  cy="8" r="2.5" fill="var(--crm-blue)" />
                <circle cx="9"  cy="5" r="2.5" fill="#22c55e" />
                <circle cx="14" cy="3" r="2.5" fill="#f59e0b" />
                <line x1="4" y1="8" x2="9" y2="5"  stroke="var(--crm-blue)" strokeWidth="1.2" />
                <line x1="9" y1="5" x2="14" y2="3" stroke="#22c55e"        strokeWidth="1.2" />
              </svg>
              <span style={{ fontSize: 17, fontWeight: 600, color: 'var(--crm-fg1)' }}>Channel Delivery Report</span>
            </div>
            <div style={{ fontSize: 12, color: 'var(--crm-fg3)', marginTop: 3 }}>
              Delivery, bounce, and opt-out rates broken down per messaging channel
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
            ...
            <button
              onClick={() => data && exportCsv(data.channels)}
        ...
        <div style={{ background: 'var(--crm-white)', border: '1px solid var(--crm-border)', borderRadius: 10, overflow: 'hidden' }}>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr>
                  ...
                  {TH('delivery_rate', 'Delivery Rate')}
                  {TH('bounce_rate',   'Bounce Rate'  )}
                  {TH('open_rate',     'Open Rate'    )}
                  {TH('ctr',           'CTR'          )}
                  {TH('opt_outs',      'Opt-outs'     )}
                </tr>
              </thead>
              <tbody>
                ...
                {!loading && sortedChannels.map((row, i) => (
                  <tr key={row.channel} style={{ background: i % 2 === 0 ? 'var(--crm-white)' : 'transparent' }}>
                    <td style={{ padding: '11px 14px', fontSize: 13, fontWeight: 500, color: 'var(--crm-fg1)', borderBottom: '1px solid var(--crm-border)' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 7 }}>
                        <span style={{ width: 9, height: 9, borderRadius: '50%', background: channelColor(row.channel), flexShrink: 0 }} />
                        {channelLabel(row.channel)}
                      </span>
                    </td>
                    <td style={{ padding: '11px 14px', fontSize: 13, color: 'var(--crm-fg2)', borderBottom: '1px solid var(--crm-border)' }}>{fmt(row.messages)}</td>
                    <td style={{ padding: '11px 14px', fontSize: 13, color: 'var(--crm-fg2)', borderBottom: '1px solid var(--crm-border)' }}>{fmtPct(row.delivery_rate)}</td>
                    <td style={{ padding: '11px 14px', fontSize: 13, color: 'var(--crm-fg2)', borderBottom: '1px solid var(--crm-border)' }}>{fmtPct(row.bounce_rate)}</td>
                    ...
                    <td style={{ padding: '11px 14px', fontSize: 13, color: 'var(--crm-fg2)', borderBottom: '1px solid var(--crm-border)' }}>
                      {row.opt_outs != null ? row.opt_outs.toLocaleString() : '—'}
                    </td>
                  </tr>
                ))}
                {!loading && sortedChannels.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ padding: 32, textAlign: 'center', fontSize: 13, color: 'var(--crm-fg4)' }}>
                      No delivery data found for the selected window
                    </td>
                  </tr>
                )}
              </tbody>
          ...
          {!loading && sortedChannels.length > 0 && (
            <div style={{ padding: '10px 18px', borderTop: '1px solid var(--crm-border)', fontSize: 12, color: 'var(--crm-fg3)' }}>
              Showing {sortedChannels.length} row{sortedChannels.length !== 1 ? 's' : ''} —{' '}
              <span style={{ color: 'var(--crm-blue)', cursor: 'pointer' }} onClick={() => data && exportCsv(data.channels)}>Export full dataset</span>
            </div>
          )}
        </div>
      </div>
