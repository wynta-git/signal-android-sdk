...
export const api = {
  ...
  async createSubhead(parentId: number, payload: Record<string, unknown>) {
    ...
    if (MOCK_HEADS[parentId]) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (MOCK_HEADS[parentId] as any).subheads = [...(MOCK_HEADS[parentId].subheads || []), { id }];
    }
  ...
  async updateConfigure(id: number, patch: Record<string, unknown>) {
    await delay(280);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (MOCK_CONFIGURES as any)[id] = { ...(MOCK_CONFIGURES as any)[id], ...patch };
  ...
  async createPromoCode(configureId: number, payload: Record<string, unknown>) {
    ...
    if (MOCK_CONFIGURES[configureId]) {
      MOCK_CONFIGURES[configureId].promo_codes = [...(MOCK_CONFIGURES[configureId].promo_codes || []), code as never];
    }
  ...
  async createTrigger(configureId: number, payload: Record<string, unknown>) {
    ...
    if (MOCK_CONFIGURES[configureId]) {
      MOCK_CONFIGURES[configureId].triggers = [...(MOCK_CONFIGURES[configureId].triggers || []), trigger as never];
    }
  ...
  async createEligibility(configureId: number, payload: Record<string, unknown>) {
    ...
    if (MOCK_CONFIGURES[configureId]) {
      MOCK_CONFIGURES[configureId].eligibilities = [...(MOCK_CONFIGURES[configureId].eligibilities || []), elig as never];
    }
  ...
  async fetchSegmentPlayers(segmentId: string | number, { page = 1, search = '' }: { page?: number; search?: string } = {}): Promise<PlayerPage> {
    await delay();
    ...
    if (q) {
      ...
      for (let i = 0; i < cap; i++) searchPool.push(makePlayer(String(segmentId), i));
      ...
      slice = [];
      for (let i = startIdx; i < endIdx; i++) slice.push(makePlayer(String(segmentId), i));
      scopeNote = '';
    }
