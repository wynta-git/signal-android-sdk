...
export default function ManualBonusForm({ state, submitting, onCancel, onSubmit }: ManualBonusFormProps) {
  const dispatch = useAppDispatch();
  const cfg = state.parentId != null ? MOCK_CONFIGURES[state.parentId] : MOCK_CONFIGURES[261];
  ...
  const [mode, setMode]           = useState<'SEGMENT' | 'UPLOAD'>('SEGMENT');
  ...
  const parsedRows: ParsedRow[] = useMemo(() => {
    ...
    return idsText.split(/\r?\n/)
      .map(line => {
        ...
        return { pid, amt: isNaN(amt ?? NaN) ? null : amt };
      })
        ...
        {mode === 'SEGMENT' && (
          <div className="field-group">
            ...
            <div className="mb-seg-grid">
              {(MANUAL_SEGMENTS as Segment[]).map(s => (
                <button
                  key={String(s.id)}
                  type="button"
                  className={'mb-seg-card' + (s.id === segmentId ? ' active' : '')}
                  onClick={() => setSegmentId(String(s.id))}
                >
                  <div className="top">
                    <span className="dot"/>
