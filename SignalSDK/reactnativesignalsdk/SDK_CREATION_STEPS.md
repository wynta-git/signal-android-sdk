...
The SDK is a **standalone TypeScript package** located inside the monorepo at `reactnativesignalsdk/`. It is built to be consumed by React Native apps and can be published to npm independently.
...
Ran the build command from inside `reactnativesignalsdk/`:
...
- How to use locally in the app (`npm install ../reactnativesignalsdk`)
...
```bash
npm install ../reactnativesignalsdk
```
